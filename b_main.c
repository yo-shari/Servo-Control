...
void writeAngleA(int angleArg) {
        ...
        p = (min16*16L*8 + (max16-min16)*(16L*8)*angleA/180L)/8L;        //calculates the duty cycle to reach the desired angle 
        OCR1A = p;                                                      //the register you assign the duty cycle too
}
...
//turns it while printing an angle
       /* while(1) {
        ...
        while(1){
                //uint16_t timeout = uart_hw_getc();
                char unicorn = uart_hw_getc();
                writeAngleA(unicorn);
                writeAngleB(unicorn);
