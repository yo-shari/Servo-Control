...
Date:Nov 20th 2015
Description: Uses PWM to turn the servos independently to the angles passed.
*****************************************************************************************/ 
...
void writeAngle(int angleArg,int * angle, char letter) {
         if (angleArg > 0){
            ...
            uart_putchar((uint8_t)angleArg);
        }
}
     /**/
void writeAngleA(int angleArg) {
        uint16_t p;
        char temp_string[8];
        ...
        p = (min16*16L*8 + (max16-min16)*(16L*8)*angleA/180L)/8L;

        OCR1A = p;
        uart_putstring("Setting servo to A:");
        itoa(readAngleA(), temp_string, 10);
...
void writeAngleB(int angleArg) {
        ...
        OCR1B = p;
        uart_putstring("Setting servo to B:");
        itoa(readAngleB(), temp_string, 10);
        uart_putstring(temp_string);
        uart_putstring("\n\r");
}
